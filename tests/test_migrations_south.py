...
def get_expected_dir():
    return os.path.normpath(
        os.path.join(get_migrations_dir(), "..", "south_migrations_expected")
    )
...
@unittest.skipIf("south" not in settings.INSTALLED_APPS, "South not in INSTALLED_APPS")
class SouthMigrationTest(TagTestManager, TransactionTestCase):
    ...
    def _import_migration(self, path, name):
        ...
        module_name = ".".join(["tests"] + path[len(root) + 1 :].split(os.sep) + [name])
    ...
    def _parse_migration_function(self, fn):
        ...
        def ast_dump(*nodes):
            ...
            return ", ".join(
                ast.dump(node).replace("Str(s=u'", "Str(s='").replace("{u'", "{'")
                for node in nodes
            )
        ...
        def parse_field(field):
            ...
            if isinstance(field.func, ast.Attribute):
                # Call to self.gf
                field_cls = six.text_type(field.func.value.id + "." + field.func.attr)
            else:
                # Direct ref to field
        ...
        for node in root.body[0].body:
            ...
            if method == "create_table":
                ...
                field_id = six.text_type(dbcall.args[0].s) + "." + str(dbcall.args[1].s)
    ...
    def assertMigrationExpected(self, name):
        ...
        self.assertEqual(
            fix_text_type(mig1.Migration.models), fix_text_type(mig2.Migration.models)
        )
    ...
    def migrate_tree(self):
        ...
        self.assertTagModel(
            model_tagged.tags.tag_model, {"one/two/three": 0, "uno/dos/tres": 0}
        )
        ...
        self.assertSequenceEqual(
            frozen_singletag["Meta"]["_bases"], ["tagulous.models.BaseTagModel"]
        )

        frozen_tags = south.creator.freezer.prep_for_freeze(model_tree.tags.tag_model)
        self.assertSequenceEqual(
            frozen_tags["Meta"]["_bases"], ["tagulous.models.BaseTagTreeModel"]
        )

        # Add in the prepared schemamigration for the tree
        migrations_dir = get_migrations_dir()
        expected_dir = get_expected_dir()
        shutil.copy(os.path.join(expected_dir, "0003_tree.py"), migrations_dir)
        ...
        self.assertTagModel(
            model_tree.tags.tag_model, {"one/two/three": 0, "uno/dos/tres": 0}
        )
    ...
    def migrate_data(self):
        ...
        model_tree.objects.create(name="Test 2", singletag="Mrs", tags="one/two")
        model_tree.objects.create(name="Test 3", singletag="Mr", tags="uno/dos")
        self.assertTagModel(model_tree.singletag.tag_model, {"Mr": 2, "Mrs": 1})
        self.assertTagModel(
            model_tree.tags.tag_model, {"one": 0, "one/two": 2, "uno": 0, "uno/dos": 2}
        )

        # Add in the datamigration
        migrations_dir = get_migrations_dir()
        expected_dir = get_expected_dir()
        shutil.copy(os.path.join(expected_dir, "0004_data.py"), migrations_dir)
